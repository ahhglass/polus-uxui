...
void main(){
  ...
  oC=vec4(col*vs,vs);
	...
	function featherAlpha(source: Float32Array, width: number, height: number, radius = 2) {
		...
		for (let y = 1; y < height - 1; y++) {
			for (let x = 1; x < width - 1; x++) {
				...
				for (const offset of [1, -1, width, -width]) {
					...
					if (shapeMask[neighbor] && !boundaryMask[neighbor]) {
						sum += values[neighbor];
						count++;
					}
				}
				if (count > 0) values[idx] = sum / count;
			}
		}
	}
	...
	function processImage(img: HTMLImageElement): ImageData {
		...
		for (let i = 0; i < size; i++) {
			...
			shapeMask[i] = alphaValues[i] > 0.35 ? 1 : 0;
		}
		const featheredAlpha = featherAlpha(alphaValues, width, height, 2);
		for (let y = 0; y < height; y++) {
			for (let x = 0; x < width; x++) {
				const idx = y * width + x;
		...
		softenBoundaryValues(u, shapeMask, boundaryMask, width, height);
		...
		for (let i = 0; i < size; i++) {
			...
			outData.data[px + 3] = Math.round(Math.min(1, featheredAlpha[i]) * 255);
		}
		return outData;
	}
</script>
...
<script lang="ts">
	type Props = {
		...
		contour?: number;
		tintColor?: string;
		/** Множитель разрешения canvas относительно CSS-размера (для чётких краёв на малых размерах) */
		renderScale?: number;
	};
	let {
		imageSrc,
	...
	$effect(() => {
		...
		const syncCanvasSize = () => {
			...
			const display = Math.max(rect.width, rect.height);
			const side = Math.max(Math.round(display * dpr * renderScale), 256);
		...
		const uploadTexture = (imgData: ImageData) => {
			...
			if (isPowerOfTwo(imgData.width) && isPowerOfTwo(imgData.height)) {
